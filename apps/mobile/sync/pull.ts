...
export async function pull(options: { full?: boolean } = {}): Promise<PullResult> {
  ...
  // Categories come down whole every time, not since the high water mark. The
  // table is a few dozen rows, and a wholesale fetch is the only way a deleted
  // or retired category leaves this device — a "changed since" query returns
  // nothing at all for a row that no longer exists.
  //
  ...
  const [products, users, categories, store] = await Promise.all([
  ...
  // PIN hashes come through a function rather than a readable column, and are
  // always fetched in full — they are what lets a cashier unlock offline.
  const pins = await fetchCashierPins(supabase);
  const pinMap = new Map(pins.map((row) => [row.id, row.pinHash]));

  await upsertProducts(products);
  await upsertUsers(users, pinMap);
  await replaceCategories(categories);
  await saveLocalStoreSettings(store);

  // Categories and the store row are deliberately absent from the mark: they are
  // fetched whole regardless, and letting a category rename push the mark
